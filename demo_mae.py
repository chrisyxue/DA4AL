...
import warnings
import torch
from utils import get_dataset, get_net, get_net_lpl, get_net_waal, get_strategy, eval_skl_model, eval_round, render_res_file
...
# get the distance matrix path
if 'Sample' in args_input.aug_strategy:
	args_input.dis_matrix_path = render_res_file_mae_dis_matrix(args_input,create=False)
	if os.path.exists(os.path.join(args_input.dis_matrix_path,'dis_mat.pt')) == False:
		raise ValueError('Produce the distance matrix first at dir ' + args_input.dis_matrix_path)
...
while (iteration > 0): 
	...
	if 'Sample' in args_input.aug_strategy:
		dataset.get_dis_mat(args_input)
	...
	if args_input.ALstrategy == 'LossPredictionLoss':
		...
		net = get_net_mae(args_input.dataset_name, args_task, device, args_input)			# load network
...
# Store Results
file_path = render_res_file_mae(args_input)
